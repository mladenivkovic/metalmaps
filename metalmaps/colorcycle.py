...
# only import whatever you are using
import numpy as np
from numpy import ndarray, linspace, array, zeros, argsort, sqrt, dot


def _scrabmle(arr: ndarray):
    """
    scramble the order of a numpy array
    """
...
def get_colorlist_from_colormap(cmap: LinearSegmentedColormap) -> List[str]:
    """
    Generate a list of 10 colors based on the provided colormap
    """

    if not isinstance(cmap, LinearSegmentedColormap):
        raise ValueError("cmap you provided is invalid data type")

    cols = [cmap.__call__(x) for x in linspace(0., 1., 10, endpoint=True)]
    cols = array(cols)
